    ...
    if (fileExist(spriteSvgPath)) {
        return src(spriteSvgPath + '*.svg')
            .pipe(
                svgmin(function () {
                    return {
                        plugins: [{ cleanupIDs: { minify: true } }, { removeViewBox: false }, { removeAttrs: { attrs: '(fill|stroke)' } }],
                    };
                })
            )
...
function copyImg() {
    return src(st.config.addAssets).pipe(dest(`${dir.build}img/`));
}
...
function compileSass() {
    const fileList = [`${dir.src}scss/style.scss`];
    return (
...
function clearBuildDir() {
    return del([`${dir.build}**/*`, `!${dir.build}readme.md`]);
}
...
function serve() {
    ...
    watch([`src/pages/**/*.pug`], { events: ['change', 'add'], delay: 100 }, series(compilePugFast, parallel(writeSassImportsFile, writeJsRequiresFile), parallel(compileSass, buildJs), reload));
    ...
    watch([`${dir.comp}**/*.pug`], { delay: 100 }, series(writePugMixinsFile, compilePug, parallel(writeSassImportsFile, writeJsRequiresFile), parallel(compileSass, buildJs), reload));
    // Шаблоны pug: все события
    watch([`${dir.src}pug/**/*.pug`, `!${dir.src}pug/mixins.pug`], { delay: 100 }, series(compilePug, parallel(writeSassImportsFile, writeJsRequiresFile), parallel(compileSass, buildJs), reload));
    // Разметка Блоков: добавление
    watch([`${dir.blocks}**/*.pug`], { events: ['add'], delay: 100 }, series(compilePug, reload));
    // Разметка Блоков: изменение
    watch([`${dir.blocks}**/*.pug`], { events: ['change'], delay: 100 }, series(compilePug, reload));

    /*
     * SCSS
     */
    // Стили Блоков: добавление
    watch([`${dir.blocks}**/*.scss`], { events: ['add'], delay: 100 }, series(writeSassImportsFile, compileSass));
    // Стили Блоков: изменение
    watch([`${dir.blocks}**/*.scss`], { events: ['change'], delay: 100 }, series(compileSass));
    // Миксины для scss: все события
    watch([`${dir.comp}**/*.scss`], { delay: 100 }, series(writeSassImportsFile, compileSass));
    ...
    watch([`${dir.src}js/global-scripts.js`, `${dir.blocks}**/*.js`, `${dir.comp}**/*.js`], { events: ['all'], delay: 100 }, series(writeJsRequiresFile, buildJs, reload));

    /*
     * Images
     */
    // Спрайт SVG
    watch([`${dir.src}img/sprite-svg/*.svg`], { events: ['all'], delay: 100 }, series(generateSvgSprite, copyImg, reload));
    // Картинки: все события
    watch(st.config.addAssets, { events: ['all'], delay: 100 }, series(copyImg, reload));
    /*
     * Статичные файлы
     */
    watch([`${dir.src}static/**/*`], { events: ['all'], delay: 100 }, series(copyStatic, reload));
}

exports.build = series(parallel(clearBuildDir, writePugMixinsFile), parallel(compilePugFast, generateSvgSprite), parallel(copyStatic, copyImg, writeSassImportsFile, writeJsRequiresFile), parallel(compileSass, buildJs));

exports.default = series(parallel(clearBuildDir, writePugMixinsFile), parallel(compilePugFast, generateSvgSprite), parallel(copyStatic, copyImg, writeSassImportsFile, writeJsRequiresFile), parallel(compileSass, buildJs), serve);
